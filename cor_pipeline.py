...
def main() -> None:
    ...
    print(f"Nemapate salvate: {unmatched_path.resolve()} (rânduri: {len(cor_unmatched)})")
    print(f"Top high: {high_path.resolve()}")
    print(f"Top medium: {medium_path.resolve()}")
    print(f"Top low: {low_path.resolve()}")
    print(f"Rânduri finale: {len(cor)}")
    print(f"Coloană COR folosită pentru nume: {cor_name_col}")
    print(f"Coloană ESCO folosită pentru nume: {esco_job_col}")
    if len(cor) > 0:
